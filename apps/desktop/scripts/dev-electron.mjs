...
const remoteDebuggingPort = (
  process.env.CONVERGEOS_DESKTOP_REMOTE_DEBUGGING_PORT ??
...
if (devProtocolClient) {
  childEnv.CONVERGEOS_DESKTOP_APP_USER_MODEL_ID = devProtocolClient.appBundleId;
  childEnv.CONVERGEOS_DESKTOP_PROTOCOL_REGISTRATION_MANAGED = "1";
}
...
function cleanupStaleDevApps() {
  ...
  NodeChildProcess.spawnSync("pkill", ["-f", "--", `--convergeos-dev-root=${desktopDir}`], {
    stdio: "ignore",
  });
}
...
function startApp() {
  ...
  const launchArgs = devProtocolClient
    ? electronArgs
    : [...electronArgs, `--convergeos-dev-root=${desktopDir}`, "dist-electron/main.cjs"];
