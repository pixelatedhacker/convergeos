// This file mostly exists because we want dev mode to say "ConvergeOS (Dev)" instead of "electron"
...
const devBundleIdSuffix = NodePath.basename(repoRoot)
  .toLowerCase()
  .replaceAll(/[^a-z0-9]+/g, "");
export const APP_DISPLAY_NAME = isDevelopment ? "ConvergeOS (Dev)" : "ConvergeOS (Alpha)";
export const APP_BUNDLE_ID = isDevelopment
  ? `com.convergeos.app.dev.${devBundleIdSuffix || "local"}`
  : "com.convergeos.app";
const APP_PROTOCOL_SCHEMES = isDevelopment
  ? ["convergeos-dev", "t3code-dev"]
  : ["convergeos", "t3code"];
...
export function makeDevelopmentEnvironmentScript(environment) {
  const envEntries = [
    ...
    ["CONVERGEOS_PORT", environment.CONVERGEOS_PORT ?? environment.T3CODE_PORT],
    ["CONVERGEOS_HOME", environment.CONVERGEOS_HOME ?? environment.T3CODE_HOME],
    [
      "CONVERGEOS_COMMIT_HASH",
      environment.CONVERGEOS_COMMIT_HASH ?? environment.T3CODE_COMMIT_HASH,
    ],
    [
      "CONVERGEOS_OTLP_TRACES_URL",
      environment.CONVERGEOS_OTLP_TRACES_URL ?? environment.T3CODE_OTLP_TRACES_URL,
    ],
    [
...
export function makeDevelopmentLauncherScript({
  ...
  return [
    ...
    `exec ${shellSingleQuote(electronBinaryPath)} --convergeos-dev-root=${shellSingleQuote(desktopRoot)} ${shellSingleQuote(mainEntryPath)} "$@"`,
    "",
  ].join("\n");
}
...
export function resolveMacBundleInfoPlistStrings(executableName) {
  return {
    ...
    NSScreenCaptureUsageDescription:
      "ConvergeOS captures the active window when you use the snapshot shortcut.",
    NSDocumentsFolderUsageDescription:
      "ConvergeOS reads project files you open in the desktop app.",
  };
}
...
function buildMacLauncher(electronBinaryPath) {
  ...
  if (isDevelopment) {
    ...
    // Electron.app even though this bundle's Info.plist has the ConvergeOS name.
