...
it.runIf(hasDbus)("captures through real D-Bus marshalling on a private bus", async () => {
  ...
  try {
    ...
    await server.requestName(
      "org.gnome.Shell.Extensions.ConvergeOSSnapShot",
    ...
    expect(await portal.backend("convergeos")).toBe("screenshot-portal");
    expect(await portal.capturePortal()).toEqual({ png });
    expect(target).toBe(8);
    portalVersion = 2;
    const extension = connect();
    expect(await extension.backend("convergeos")).toBe("gnome-extension");
    expect(await extension.captureExtension("convergeos")).toMatchObject({
      png,
      window: { processId: 42 },
    });
    ...
    expect(await updated.backend("convergeos")).toBe("gnome-extension");
    const snapshot = await updated.captureExtension("convergeos", {
      flash: true,
      animate: true,
    });
    expect(snapshot.feedback?.animationStarted).toBe(true);
    await snapshot.feedback!.activate("ConvergeOS");
    ...
    expect(activateTitle).toBe("ConvergeOS");
    expect(animateFrame).toEqual([0.1, 0.8, 0.2, 0.1]);
    vi.stubEnv("XDG_CURRENT_DESKTOP", "KDE");
    const kde = connect();
    expect(await kde.backend("convergeos")).toBe("kde");
