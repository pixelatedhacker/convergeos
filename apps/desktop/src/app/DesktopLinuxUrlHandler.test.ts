...
const makeEnvironment = (overrides: Record<string, unknown> = {}) =>
  DesktopEnvironment.DesktopEnvironment.of({
    ...
    isPackaged: true,
    isDevelopment: false,
    displayName: "ConvergeOS (Alpha)",
    linuxDesktopEntryName: "convergeos.desktop",
    linuxWmClass: "convergeos",
    linuxApplicationsDir: "/home/alice/.local/share/applications",
    appImagePath: Option.some("/home/alice/Applications/ConvergeOS.AppImage"),
...
describe("DesktopLinuxUrlHandler", () => {
  it("renders a scheme-handler desktop entry with freedesktop Exec quoting", () => {
    ...
    assert.include(entry, "MimeType=x-scheme-handler/convergeos;");
  });
  ...
  it("carries structured context on registration errors", () => {
    const writeError = new DesktopLinuxUrlHandler.DesktopLinuxUrlHandlerRegistrationError({
      ...
      scheme: "convergeos",
      desktopEntryPath: "/home/alice/.local/share/applications/convergeos.desktop",
      cause: new Error("boom"),
    });
    assert.equal(
      writeError.message,
      "Failed to register the convergeos:// URL handler (step: write-desktop-entry).",
    );
    assert.equal(
      writeError.desktopEntryPath,
      "/home/alice/.local/share/applications/convergeos.desktop",
    );
    ...
    assert.equal(
      exitError.message,
      "Failed to register the convergeos:// URL handler (step: set-default-handler, xdg-mime exit code 4).",
    );
  });
  ...
  it.effect("writes the handler entry and claims the scheme default via xdg-mime", () => {
    ...
    return Effect.gen(function* () {
      ...
      assert.include(
        ...
        'Exec="/home/alice/Applications/ConvergeOS.AppImage" %U',
      );
      assert.include(recorded.files[0]?.content, "MimeType=x-scheme-handler/convergeos;");
      assert.deepEqual(recorded.commands, [
        {
          command: "xdg-mime",
          args: ["default", "convergeos.desktop", "x-scheme-handler/convergeos"],
        },
      ]);
    });
  });
    ...
    return Effect.gen(function* () {
      yield* runRegister(recorded, {
        existingEntry: DesktopLinuxUrlHandler.renderUrlHandlerDesktopEntry({
          displayName: "ConvergeOS (Alpha)",
          execTarget: "/home/alice/Applications/ConvergeOS.AppImage",
          scheme: "convergeos",
        }),
      });
  ...
  it.effect("writes the portal identity without claiming the URL scheme in development", () => {
    ...
    return Effect.gen(function* () {
      ...
      assert.equal(
        ...
        "/home/alice/.local/share/applications/convergeos-dev.desktop",
      );
    ...
    return Effect.gen(function* () {
      ...
      yield* runRegister(writeFailed, {
        writeError: PlatformError.systemError({
          ...
          pathOrDescriptor: "/home/alice/.local/share/applications/convergeos.desktop",
        }),
      });
