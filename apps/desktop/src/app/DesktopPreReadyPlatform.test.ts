  ...
  for (const previousEntry of [undefined, 'Exec="/Applications/deleted-previous.AppImage" %U']) {
    it.effect(
      ...
      () => {
        ...
        let desktopName = "convergeos.desktop";
        ...
        writeFileSyncMock.mockImplementation((path: string, contents: string) => {
          if (path === "/xdg/applications/convergeos.desktop") desktopEntry = contents;
        });
        ...
        return Effect.scoped(
          Effect.gen(function* () {
            ...
            assert.equal(identity.desktopName, "convergeos.desktop");
            assert.include(identity.desktopEntry ?? "", 'Exec="/Applications/current.AppImage" %U');
            assert.include(identity.desktopEntry ?? "", "Name=ConvergeOS (Alpha)");
            assert.include(identity.desktopEntry ?? "", "MimeType=x-scheme-handler/convergeos;");
          }),
