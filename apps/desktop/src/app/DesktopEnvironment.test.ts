...
const defaultInput = {
  ...
  processArch: "arm64",
  appVersion: "0.0.22",
  appPath: "/Applications/ConvergeOS.app/Contents/Resources/app.asar",
  isPackaged: false,
  resourcesPath: "/Applications/ConvergeOS.app/Contents/Resources",
...
describe("DesktopEnvironment", () => {
  it.effect("derives state paths and development identity inside Effect", () =>
    Effect.gen(function* () {
      ...
      assert.equal(environment.appUserModelId, "com.convergeos.app.dev");
      assert.equal(environment.linuxWmClass, "convergeos-dev");
      assert.equal(environment.linuxDesktopEntryName, "convergeos-dev.desktop");
      assert.deepEqual(
  ...
  it.effect("uses the stable desktop entry as the packaged Linux portal identity", () =>
    Effect.gen(function* () {
      ...
      assert.equal(environment.linuxDesktopEntryName, "convergeos.desktop");
    }),
  );
  ...
  it.effect("keeps implicit development state separate from production state", () =>
    Effect.gen(function* () {
      ...
      assert.equal(development.stateDir, "/Users/alice/.convergeos/dev");
      assert.equal(production.stateDir, "/Users/alice/.convergeos/userdata");
    }),
  );
  ...
  it.effect("uses a configured app user model id override", () =>
    Effect.gen(function* () {
      const environment = yield* makeEnvironment(
        {},
        {
          CONVERGEOS_DESKTOP_APP_USER_MODEL_ID: " com.convergeos.app.dev.local ",
          VITE_DEV_SERVER_URL: "http://localhost:5173",
        },
      );

      assert.equal(environment.appUserModelId, "com.convergeos.app.dev.local");
    }),
  );
