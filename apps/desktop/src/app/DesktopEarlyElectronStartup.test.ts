...
describe("DesktopEarlyElectronStartup", () => {
  ...
  it("keeps implicit development state under ~/.convergeos/dev when CONVERGEOS_HOME is unset", () => {
    const preference = resolveEarlyLinuxPasswordStorePreference({
      env: {
        VITE_DEV_SERVER_URL: "http://127.0.0.1:5173",
      },
      ...
      joinPath,
      readFileString: (path) => {
        assert.equal(path, "/home/user/.convergeos/dev/desktop-settings.json");
  ...
  it("treats whitespace-only CONVERGEOS_HOME as unconfigured in development", () => {
    const preference = resolveEarlyLinuxPasswordStorePreference({
      env: {
        CONVERGEOS_HOME: "   ",
        VITE_DEV_SERVER_URL: "http://127.0.0.1:5173",
