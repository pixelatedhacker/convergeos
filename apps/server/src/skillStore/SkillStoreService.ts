...
export type ResolveProjectCwd = (projectId: ProjectId) => Effect.Effect<string, SkillStoreError>;
...
export class SkillStoreService extends Context.Service<
  SkillStoreService,
  {
    ...
    ) => Effect.Effect<SkillStoreSearchResult, SkillStoreError>;
    readonly getDetail: (
      input: SkillStoreDetailInput,
    ) => Effect.Effect<SkillStoreDetail, SkillStoreError>;
    readonly listInstalled: Effect.Effect<SkillStoreListResult, SkillStoreError>;
    readonly install: (
      input: SkillStoreInstallInput,
      resolveProjectCwd: ResolveProjectCwd,
    ) => Effect.Effect<InstalledSkill, SkillStoreError>;
    readonly uninstall: (
      input: SkillStoreUninstallInput,
      resolveProjectCwd: ResolveProjectCwd,
    ) => Effect.Effect<SkillStoreListResult, SkillStoreError>;
...
const make = Effect.gen(function* () {
  ...
  const resolveTargetCwd = (
    ...
    resolveProjectCwd: ResolveProjectCwd,
  ): Effect.Effect<string, SkillStoreError> => {
  ...
  const uninstall = Effect.fn("SkillStoreService.uninstall")(function* (
    ...
    return yield* mutationLock.withPermits(1)(
      Effect.gen(function* () {
        ...
        let firstFailure: SkillStoreError | undefined;
...
export const layerTest = (overrides?: Partial<SkillStoreService["Service"]>) =>
  Layer.succeed(
    ...
    SkillStoreService.of({
      ...
      getDetail: () =>
        Effect.fail(new SkillStoreError({ reason: "notFound", detail: "not implemented in test" })),
      listInstalled: Effect.succeed({ skills: [] }),
      install: () =>
        Effect.fail(
          new SkillStoreError({ reason: "installFailed", detail: "not implemented in test" }),
        ),
