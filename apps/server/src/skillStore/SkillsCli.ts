...
import { SkillStoreError, type SkillStoreHarness, type SkillStoreScope } from "@t3tools/contracts";
 ...
 * `SkillStoreError`s; stderr is included in the detail because the CLI puts
...
const scopeArgs = (scope: SkillStoreScope): ReadonlyArray<string> =>
  scope === "global" ? ["--global"] : [];
...
export const installSkill = (input: {
  ...
  readonly scope: SkillStoreScope;
  readonly cwd: string;
}) =>
  runSkillsCli(
    {
