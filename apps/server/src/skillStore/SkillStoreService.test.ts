...
import { ProjectId, RegistrySkillStoreError } from "@t3tools/contracts";
  ...
  it.effect("a failed target still persists the targets that already landed", () =>
    withConfig((config) =>
      Effect.gen(function* () {
        ...
        assert.instanceOf(result.installExit, RegistrySkillStoreError);
  ...
  it.effect("setHarnessEnabled on an unknown skill fails notFound", () =>
    withConfig((config) =>
      Effect.gen(function* () {
        ...
        assert.instanceOf(error, RegistrySkillStoreError);
