...
function isKanbanCardEvent(event: OrchestrationEvent): event is KanbanCardEvent {
  return (
    ...
    event.type === "kanban.card-delegation-completed" ||
    event.type === "kanban.card-deleted"
  );
}

type KanbanDelegationEvent = Extract<
  OrchestrationEvent,
  {
    type:
...
function isKanbanDelegationEvent(event: OrchestrationEvent): event is KanbanDelegationEvent {
  if (
    event.type !== "delegation.requested" &&
    event.type !== "delegation.provision-started" &&
    event.type !== "delegation.target-bound" &&
    event.type !== "delegation.turn-requested" &&
    event.type !== "delegation.turn-bound" &&
  ...
  return event.payload.delegation.requester.kind === "kanban";
}

function projectKanbanDelegationEvent(event: KanbanDelegationEvent): KanbanBoardStreamItem {
...
const makeWsRpcLayer = (
  ...
  WsRpcGroup.toLayer(
    Effect.gen(function* () {
      ...
      const threadLaunchService = yield* ThreadLaunchService.make;
            ...
            Effect.gen(function* () {
                ...
                projectionSnapshotQuery.getCommandReadModel().pipe(
                  Effect.map((readModel) => ({
                    ...
                    delegations: (readModel.delegations ?? [])
                      .filter(
                        (delegation) =>
                          delegation.projectId === input.projectId &&
              ...
              const liveBoardEvents = liveEvents.pipe(
                Stream.filterMap((event) => {
                  if (isKanbanCardEvent(event) && kanbanEventProjectId(event) === input.projectId) {
                    return Result.succeed(projectKanbanCardEvent(event));
                  }
                  if (
                    isKanbanDelegationEvent(event) &&
                    event.payload.delegation.projectId === input.projectId
                  ) {
                    return Result.succeed(projectKanbanDelegationEvent(event));
                  }
