...
const makeProviderService = Effect.fn("makeProviderService")(function* (
  ...
  /**
   * Resolve the project-scoped browser and device capabilities attached to
   * the ConvergeOS MCP credential minted below.
   *
  ...
  const agentAccessCapabilities = (threadId: ThreadId) =>
    Effect.gen(function* () {
      ...
      const settings = yield* serverSettings.getSettings;
      if (settings.enableAgentUsageAccess) capabilities.add("usage.read");
      if (settings.enableAgentMeshAccess) {
        capabilities.add("agents.read");
        capabilities.add("agents.send");
        capabilities.add("agents.control");
      }
      if (settings.agentKanbanAccess === "read" || settings.agentKanbanAccess === "write") {
        capabilities.add("kanban.read");
      }
      if (settings.agentKanbanAccess === "write") capabilities.add("kanban.write");
      return capabilities;
    }).pipe(
      Effect.catch((cause) =>
        Effect.logWarning(
          "Could not read server settings; withholding optional agent MCP capabilities for this session.",
          { cause },
        ).pipe(Effect.as(new Set<McpInvocationContext.McpCapability>(["pull-requests"]))),
      ),
    );
    ...
    function* (threadId, rawInput) {
      ...
      return yield* Effect.gen(function* () {
        ...
        if (!instanceInfo.enabled) {
          return yield* toValidationError(
            "ProviderService.startSession",
            `Provider instance '${resolvedInstanceId}' is disabled in ConvergeOS settings.`,
          );
        }
