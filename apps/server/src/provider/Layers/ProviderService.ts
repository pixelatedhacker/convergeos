...
const makeProviderService = Effect.fn("makeProviderService")(function* (
   ...
   */
  const agentMcpCapabilities = Effect.fn("ProviderService.agentMcpCapabilities")(function* (
    threadId: ThreadId,
  ) {
    const settings = yield* serverSettings.getSettings;
    const capabilities = new Set<McpCapability>();

    let browserEnabled = settings.enableAgentBrowserAccess;
    if (Object.keys(settings.projectAgentBrowserAccessOverrides).length > 0) {
      ...
      if (Option.isNone(projectionQuery)) {
        ...
        browserEnabled = Option.isNone(thread)
          ? false
  ...
  const prepareMcpSession = (threadId: ThreadId, providerInstanceId: ProviderInstanceId) =>
    Effect.gen(function* () {
      const capabilities = yield* agentMcpCapabilities(threadId).pipe(
        Effect.catch((cause) =>
          Effect.logWarning(
