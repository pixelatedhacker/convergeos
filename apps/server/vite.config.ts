...
export default mergeConfig(
  ...
  defineConfig({
    ...
    pack: {
      ...
      define: {
        __CONVERGEOS_BUILD_CHANNEL__: JSON.stringify(cliBuildChannel),
        __CONVERGEOS_BUILD_RELAY_URL__: JSON.stringify(
          repoEnv.CONVERGEOS_RELAY_URL?.trim() ?? "",
        ),
        __CONVERGEOS_BUILD_CLERK_PUBLISHABLE_KEY__: JSON.stringify(
          repoEnv.CONVERGEOS_CLERK_PUBLISHABLE_KEY?.trim() ?? "",
        ),
        __CONVERGEOS_BUILD_CLERK_CLI_OAUTH_CLIENT_ID__: JSON.stringify(
          repoEnv.CONVERGEOS_CLERK_CLI_OAUTH_CLIENT_ID?.trim() ?? "",
        ),
        __CONVERGEOS_BUILD_RELAY_CLIENT_OTLP_TRACES_URL__: JSON.stringify(
          repoEnv.CONVERGEOS_RELAY_CLIENT_OTLP_TRACES_URL?.trim() ?? "",
        ),
        __CONVERGEOS_BUILD_RELAY_CLIENT_OTLP_TRACES_DATASET__: JSON.stringify(
          repoEnv.CONVERGEOS_RELAY_CLIENT_OTLP_TRACES_DATASET?.trim() ?? "",
        ),
        __CONVERGEOS_BUILD_RELAY_CLIENT_OTLP_TRACES_TOKEN__: JSON.stringify(
          repoEnv.CONVERGEOS_RELAY_CLIENT_OTLP_TRACES_TOKEN?.trim() ?? "",
        ),
      },
    },
    test: {
