...
> [!WARNING]
> ConvergeOS Mobile is currently in development and is not distributed yet. If you want to try it out, you can build it from source.
...
- `development`: Expo dev client, installable side-by-side as `ConvergeOS Dev`
- `preview`: persistent internal preview build, installable side-by-side as `ConvergeOS Preview`
- `production`: store/release build as `ConvergeOS`
...
```bash
CONVERGEOS_IOS_PERSONAL_TEAM=1 \
CONVERGEOS_IOS_PERSONAL_TEAM_BUNDLE_ID=com.example.t3code.dev \
vp run ios:dev
```
...
```bash
CONVERGEOS_IOS_PERSONAL_TEAM=1 \
CONVERGEOS_IOS_PERSONAL_TEAM_BUNDLE_ID=com.example.t3code \
vp run ios:release
```
...
For preview or production EAS environments, set `CONVERGEOS_CLERK_PUBLISHABLE_KEY`,
`CONVERGEOS_CLERK_JWT_TEMPLATE`, and `CONVERGEOS_RELAY_URL`
