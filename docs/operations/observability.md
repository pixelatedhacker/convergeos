...
ConvergeOS has one server-side observability model:
...
```bash
export CONVERGEOS_OTLP_TRACES_URL=http://localhost:4318/v1/traces
export CONVERGEOS_OTLP_METRICS_URL=http://localhost:4318/v1/metrics
export T3CODE_OTLP_LOGS_URL=http://localhost:4318/v1/logs
export CONVERGEOS_OTLP_SERVICE_NAME=t3-local
```

Optional:

```bash
export CONVERGEOS_TRACE_MIN_LEVEL=Info
export CONVERGEOS_TRACE_TIMING_ENABLED=true
```
...
Launch the actual app executable from the same shell so the desktop app and embedded backend inherit `CONVERGEOS_OTLP_*`.

macOS app bundle example:

```bash
CONVERGEOS_OTLP_TRACES_URL=http://localhost:4318/v1/traces \
CONVERGEOS_OTLP_METRICS_URL=http://localhost:4318/v1/metrics \
T3CODE_OTLP_LOGS_URL=http://localhost:4318/v1/logs \
CONVERGEOS_OTLP_SERVICE_NAME=t3-desktop \
"/Applications/ConvergeOS.app/Contents/MacOS/ConvergeOS"
```
...
```bash
TRACE_FILE="${CONVERGEOS_HOME:-$HOME/.t3}/userdata/logs/server.trace.ndjson"
```
...
- `CONVERGEOS_OTLP_TRACES_URL` was not set
...
- `CONVERGEOS_TRACE_FILE`: override trace file path
- `CONVERGEOS_TRACE_MAX_BYTES`: per-file rotation size, default `10485760`
- `CONVERGEOS_TRACE_MAX_FILES`: rotated file count, default `10`
- `CONVERGEOS_TRACE_BATCH_WINDOW_MS`: flush window, default `200`
- `CONVERGEOS_TRACE_MIN_LEVEL`: minimum trace level, default `Info`
- `CONVERGEOS_TRACE_TIMING_ENABLED`: enable timing metadata, default `true`

OTLP export:

- `CONVERGEOS_OTLP_TRACES_URL`: OTLP trace endpoint
- `CONVERGEOS_OTLP_METRICS_URL`: OTLP metric endpoint
- `T3CODE_OTLP_LOGS_URL`: OTLP log endpoint
- `CONVERGEOS_OTLP_EXPORT_INTERVAL_MS`: export interval, default `10000`
- `CONVERGEOS_OTLP_SERVICE_NAME`: service name, default `t3-server`
- `CONVERGEOS_OTLP_HEADERS`: extra headers for all three exporters, same format as
  `OTEL_EXPORTER_OTLP_HEADERS`: comma-separated `key=value` pairs with percent-encoded values.
- `CONVERGEOS_OTLP_PROTOCOL`: `http/json` (default) or `http/protobuf`
