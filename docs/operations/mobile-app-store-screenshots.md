...
1. Creates three temporary T3 base directories and starts a local server for each on an available
   port.
2. Creates ConvergeOS, React, and Linux Git repositories with recognizable favicons, feature branches,
   and a deterministic ConvergeOS review diff.
...
The Pending rows use the production offline outbox and point at the real ConvergeOS and React fixture
