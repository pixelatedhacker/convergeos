...
machine running your ConvergeOS server does that, its themes appear in the theme library alongside
your own, and ConvergeOS follows them: change the desktop theme and ConvergeOS retints with it, without
a restart.
...
follows the machine it is connected to, not the device it runs on. ConvergeOS Mobile keeps its own
...
the standard ConvergeOS look. A theme you saved always wins over a published one with the same id.
...
A machine publishes themes by writing files into the `themes` directory of the ConvergeOS state
...
exported from ConvergeOS (the **Download** button's output) works as-is, so any theme someone shared
...
`appearance` is `light` or `dark`, and `canvas` and `accent` are hex colors. ConvergeOS generates the
rest of the palette from those two, the same way the guided theme editor does, so the result reads
as a ConvergeOS theme wearing your desktop's colors rather than a transplant of another app's.
...
out keep their generated value. A role this version of ConvergeOS does not know is ignored rather
...
Write each file atomically — write a temporary file beside it and rename — so ConvergeOS never reads
