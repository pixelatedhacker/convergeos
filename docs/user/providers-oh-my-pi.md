...
T3 Code runs [Oh My Pi](https://github.com/can1357/oh-my-pi) through its Agent Client
...
Install Oh My Pi on the environment that runs T3 Code. Follow the
...
**Oh My Pi**. T3 Code uses `omp` from the environment's `PATH` by default. Set **Binary path** if
the executable is elsewhere.

Oh My Pi is off by default. T3 Code does not install or authenticate Oh My Pi. When T3 Code can
...
Select **Default** to keep the model selected by the Oh My Pi session. T3 Code never sends
`default` as an ACP model ID. A resumed thread keeps its native Oh My Pi session and model.

Refresh provider status after you change Oh My Pi credentials or model configuration. T3 Code runs
...
T3 Code starts one `omp acp` process for each active thread and maps permission modes as follows:

| T3 Code mode      | Oh My Pi approval mode | Behavior                                                         |
| ----------------- | ---------------------- | ---------------------------------------------------------------- |
...
for the limits of each T3 Code mode.

## Project tools and remote clients

Oh My Pi receives the thread's project-scoped T3 Code MCP server. This gives the agent the same
...
T3 Code sends images through ACP when the selected model supports them. Other attachment formats
follow the provider's normal attachment behavior.

## Troubleshoot setup

If T3 Code cannot find Oh My Pi, run `omp --version` on the environment and check **Binary path**.
