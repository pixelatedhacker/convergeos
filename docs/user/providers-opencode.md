...
ConvergeOS uses the OpenCode setup on the connected environment. With a remote environment, its
OpenCode login and configuration apply, not the setup on your desktop or phone.

ConvergeOS requires OpenCode 1.14.19 or newer. It checks the server version before it loads models or
...
Without a server URL, ConvergeOS starts a local OpenCode server. The process inherits
`OPENCODE_SERVER_PASSWORD` from the environment. A password in the provider settings overrides
that environment value for both the local process and ConvergeOS.

With a server URL, ConvergeOS connects to that external server and uses only the password in the
...
ConvergeOS waits for this cleanup before it marks the turn as stopped or sends the next prompt. It
does not stop unrelated OpenCode sessions.

Stop reports an error if OpenCode cannot list or stop a child session. When ConvergeOS closes an
OpenCode session, it also tries to stop the child sessions, but this teardown is best effort.

## Refresh the model list

ConvergeOS loads the model list when an enabled OpenCode provider starts and keeps the list in its
...
After changing an OpenCode login or configuration outside ConvergeOS, open **Settings > Providers**,
select the environment, and choose **Refresh provider status**. Changing the provider's
configuration in ConvergeOS also replaces that provider connection.
...
ConvergeOS does not own an external OpenCode server. Native configuration changes on that server can
require its own reload or restart before a refresh returns the new list.

If a refresh fails, ConvergeOS keeps the last known models, slash commands, and skills. Fix the
...
until the model metadata returns. ConvergeOS does not switch the thread to the first model in the
list.
