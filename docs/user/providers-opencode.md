...
ConvergeOS requires OpenCode 1.14.19 or newer, including when you connect an existing
OpenCode server.

## Local or external server

Leave **Server URL** empty to let ConvergeOS start OpenCode locally. A password in
provider settings applies to both that server and ConvergeOS's connection. With no
...
settings. ConvergeOS uses only that configured password for an external server; it
...
restart before ConvergeOS can see configuration changes.
