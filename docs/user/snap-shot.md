...
Shift keys together. ConvergeOS attaches the image to your draft and brings itself forward. If no thread
is open it starts a draft in the current project.

Pressing the shortcut while ConvergeOS is in front captures ConvergeOS itself.
...
a modifier pair such as Command+Command or Ctrl+Ctrl, or a key chord. ConvergeOS refuses shortcuts
...
terminal contents. If an app is slow to answer, ConvergeOS attaches the screenshot without the data
rather than waiting.
...
**GNOME.** Install the bundled **ConvergeOS SnapShots** extension during setup. It is installed
...
shortcut does not fire, check ConvergeOS under **System Settings** > **Keyboard** > **Shortcuts**. If a
...
**Review changes**. ConvergeOS shows the exact change it will make to your Hyprland config. **Save
...
**Niri.** Choose a shortcut and select **Review changes**. ConvergeOS shows the exact binding it will
...
**Other Wayland desktops.** ConvergeOS uses the desktop's screenshot portal when it can capture the
