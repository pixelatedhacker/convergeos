...
On web and desktop, choose **Create theme** to adjust a palette, or import a ConvergeOS or VS Code
...
takes precedence. If the server stops publishing the selected theme, ConvergeOS falls back to its
standard theme.
...
Save a theme exported from ConvergeOS into `~/.t3/userdata/themes/` on the server, or the `themes`
...
Set `appearance` to `light` or `dark` and supply hex colors for `canvas` and `accent`. ConvergeOS
