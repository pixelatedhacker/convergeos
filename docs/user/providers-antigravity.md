...
ConvergeOS runs Google's official Antigravity ACP agent. By default it signs in with your personal
...
not sign in this ConvergeOS provider.
...
2. Choose **Install Antigravity**. ConvergeOS downloads the official runtime from Google to that
...
6. Wait for ConvergeOS to confirm sign-in and load the model choices. Select an Antigravity model
...
machine as the ConvergeOS environment, sign-in can finish directly.
...
return URL field in ConvergeOS. Choose **Continue** on web or desktop, or **Complete sign-in** on
mobile. Do not change the address to your server's hostname.

Return to the same ConvergeOS client and environment where you started sign-in. Another client
...
a thread or bug report. ConvergeOS waits for Google's agent to confirm sign-in. A successful
...
are stored in plain text in ConvergeOS settings on that environment and are passed only to the
...
ConvergeOS does not download it until you choose to install it.

On web or desktop, **Update Antigravity** appears when ConvergeOS has a newer managed release.
...
A nonempty **Binary path** takes priority. With the field empty, ConvergeOS uses its managed
runtime, then an installation on the environment's `PATH`. An invalid explicit path reports
an error instead of selecting another installation. ConvergeOS does not update or remove manual
installations. Clear **Binary path** to use managed installation controls.

## Models and threads

The current official ACP exposes Gemini models only. ConvergeOS uses the model IDs and names
...
ConvergeOS asks you to select an available model instead of silently changing it.

Use Antigravity's native `/plan` command to request a plan. ConvergeOS's separate Plan mode control
is not available for this provider.

Antigravity reads and edits workspace files through ConvergeOS. Each write shows up as a file
...
When the agent offers **Allow for this thread** on a shell or web tool, ConvergeOS shows Google's
...
ConvergeOS keeps thread history and file diffs. Antigravity does not support conversation rewind,
...
installation. ConvergeOS refuses removal while the runtime is in use. An explicit **Binary path**
that points into the managed runtime must be cleared first.

## Account access and errors

Google controls account eligibility, models, and usage limits. ConvergeOS does not report your
...
Use any retry time Google supplies. ConvergeOS does not switch to an API key to get past the limit.
