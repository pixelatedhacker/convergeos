...
ConvergeOS runs Google's official Antigravity ACP agent on your selected environment.
...
**Sign in with Google** and complete the browser sign-in. Wait for ConvergeOS to confirm
...
hostname. Only that ConvergeOS sign-in session can finish the attempt. If it expires,
...
Use Antigravity's native `/plan` command for planning. ConvergeOS's separate Plan mode
...
ConvergeOS keeps conversation history and file diffs, but Antigravity cannot rewind
...
Put project skills in `.agents/skills`. ConvergeOS also reads `.gemini/skills` and the
