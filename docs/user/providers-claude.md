...
This guide is for people who want to use more than one Claude setup in ConvergeOS. For Codex, see
[Codex](./providers-codex.md). For first-time setup, see [Install ConvergeOS](./install.md).
...
In ConvergeOS Settings, your Claude provider can stay like this:
...
An empty `CLAUDE_CONFIG_DIR path` means ConvergeOS uses Claude Code's normal config directory.

When you set this field, ConvergeOS points Claude Code at that directory with the
...
On web and desktop, when you return to an older Claude thread with a large context, ConvergeOS
...
ConvergeOS looks for Claude skills in the Claude config directory's `skills` folder and
...
`~/.claude_personal_home/.claude`, which is not where ConvergeOS looks.

Then add another Claude provider in ConvergeOS:
...
ConvergeOS only offers Claude providers that use the same config directory for an existing thread. A
...
multiple files under its config directory, so ConvergeOS keeps separate config directories isolated
...
Add or edit a Claude provider in ConvergeOS Settings:
...
Mark `ANTHROPIC_AUTH_TOKEN` as sensitive. ConvergeOS stores the value as a server secret and does not
...
ConvergeOS does not need a special Claude Code Router provider. Treat the router as a Claude
