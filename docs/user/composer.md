...
Messages can contain up to 120,000 characters. If a draft is longer, ConvergeOS keeps it in the
...
ConvergeOS from other apps through the system share sheet. Mobile keeps a local copy of each draft
...
ConvergeOS remembers the last provider, model, and model options you selected and reuses that
...
Model options shown as provider defaults remain display values until you choose them in ConvergeOS.
ConvergeOS only sends options you selected explicitly, so an unset reasoning level or service tier can
...
readable and ConvergeOS shows a warning.
...
On web, desktop, and mobile, select a link to an image or video to open it inside ConvergeOS.
Workspace image and video links open the file viewer. Links to media outside the workspace
open a media preview.
Videos opened from the file explorer or a file-viewer tab also play inside ConvergeOS. They
...
ConvergeOS serves the original file without adding it to attachment storage. If that file is moved
...
runs in an isolated frame with no access to your ConvergeOS session. On desktop, the integrated
...
project exists there. If it does not, ConvergeOS selects another environment that has the project.
...
and attachments. ConvergeOS deletes the local audio file after transcription or cancellation. It sends
...
also reports that skill as a native slash command, ConvergeOS hides the duplicate native entry and keeps
the `/skill:Skill Name` label.

A skill token runs the skill wherever it sits in your message. ConvergeOS sends it to each provider in
...
them only there. ConvergeOS's own commands, such as `/model` and `/plan`, and skills stay available on
any line.

On desktop, press `Cmd+Enter` on macOS or `Ctrl+Enter` on Windows and Linux from a new thread to
start it in the background. ConvergeOS opens another new thread and shows an **Open** action for the
