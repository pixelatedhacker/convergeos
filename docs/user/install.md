...
ConvergeOS is a web and desktop GUI for running coding agents on your machine.

## Requirements

Node.js `^22.16 || ^23.11 || >=24.10` on the machine that runs the ConvergeOS server.

At least one provider runtime, installed and authenticated. You can install Antigravity from
ConvergeOS settings. See [Providers](#providers) below.
...
This starts the ConvergeOS server on your machine and opens the local web app. Use
`npx t3@latest --help` for the full CLI reference.

## Open a project in the desktop app

When the ConvergeOS desktop app is running on the same machine, open the current directory with:
...
thread. It does not launch the desktop app, open a browser, or start a ConvergeOS server. A background
...
successful launch, ConvergeOS keeps the current runtime and one previous runtime for rollback and
removes older caches automatically. If a cached runtime stops working, ConvergeOS launches from the
application files under `/mnt/c` instead and reinstalls the runtime on the next launch.

## Providers

ConvergeOS uses provider runtimes but does not bundle them. Install and authenticate each
provider's CLI, or use ConvergeOS's managed setup for Antigravity.
...
| Antigravity     | [Official ACP agent](https://github.com/agentclientprotocol/registry/blob/main/antigravity-acp/agent.json) | Managed by ConvergeOS | **Sign in with Google** in ConvergeOS |
...
ConvergeOS looks for, but authenticate with `agent login`, not `cursor-agent login`.
...
Run CLI login commands on the machine running the ConvergeOS server, not on the device you browse
from. Antigravity uses its sign-in controls in ConvergeOS instead of a CLI login command.
...
ConvergeOS. You can install ConvergeOS, open it, and add providers afterwards. A provider that is not
...
- [Permission modes](./permission-modes.md): how much ConvergeOS asks before acting
- [Remote access](./remote-access.md): connect from a phone, tablet, or another desktop
- [Keeping ConvergeOS in sync](./updating.md): client and server version skew
