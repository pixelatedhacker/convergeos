...
This guide is for people who want to use more than one Codex account in ConvergeOS. For Claude, see
[Claude](./providers-claude.md). For first-time setup, see [Install ConvergeOS](./install.md).
...
issue. ConvergeOS uploads the thread and Codex logs to OpenAI and shows a thread ID that you can copy
...
reports them. If Codex does not report either value, ConvergeOS leaves it out instead of using the
...
When a Codex tool needs access to an app such as Safari, ConvergeOS shows the app name and asks for
...
In ConvergeOS Settings, name it something obvious:
...
In ConvergeOS Settings, add another Codex provider:
...
ConvergeOS shows the authenticated email for providers that report one. Emails are blurred by default;
...
If you add a third Codex provider with a completely different `CODEX_HOME path`, ConvergeOS treats it
