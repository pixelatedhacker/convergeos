...
Source in `apps/desktop/gnome-extension`, UUID `snap-shot@convergeos.app`. GNOME only discovers a newly
...
The extension trusts callers that own `convergeos.SnapShot` (or the `convergeos-dev.SnapShot`
variant) on the same connection. Legacy T3 Code names remain accepted for compatibility. This is
GNOME's trusted-session-client pattern and a collision guard, not authentication against a hostile
process running as the same desktop user. Disabling the extension revokes its capture API.

Electron does not position overlay windows on Wayland, so the flash and flight run as Shell actors
inside the extension with coordinates relative to ConvergeOS's content area. Electron 44's restored-session
